...
typedef double (*gravity)(double, double, double, double, long);
extern gravity AccelFxns[N_GRAVS][N_GRAVS];
extern gravity AccelSplines[N_GRAVS][N_GRAVS];

typedef void (*latforce)(int, int, int, double *, double *);
...
{
  /* KC 8/9/14 Note that the center of mass is no longer sufficient for calculating the long range interaction at 
     monopole order if there are multiple types of gravitational interactions.  What must be kept is a center of 
     mass for each type.  Then point interactions with this node can be computed based on type. */

  FLOAT s[3][N_GRAVS];                     /*!< center-of-mass coordinates */
  FLOAT vs[3][N_GRAVS];                    /*!< center-of-mass velocities */
  FLOAT mass[N_GRAVS];                     /*!< mass of node */
#ifdef NGRAVS_ACCUMULATOR
  long Nparticles[N_GRAVS];
#endif
  /* KC 8/13/14 CM is not a sufficient characterization for all forces.  
   *
   * This can be see in the BAM case.  Given 100 BAM halos of identical mass centered at the same location,
   * their effect at large r is to contribute 100x the effect of 1 BAM halo of that mass.  However, if we 
   * just use the summed mass to determine the effective BAM halo, the scale radius is amplified 100x!  This
   * is sufficient to destroy the long distance Newtonian characterization of the BAM force and ruin the tree
   * computation.
   *
   * To combat this effect, we introduce a set of parameters that can be tailored by the end user to make
   * a more faithful effective object out of the pseudo-particle representing their non-Newtonian force.
   *
   * If multiple interactions require distinct opening parameters, then can be referenced by hard offset
   * into this array.  This is done so that, in the common case of regular Newtonian, we don't unnecessarily
   * increase the size of a node by sizeof(FLOAT)*OPENINGPARAMS
   */
...
extern struct global_data_all_processes
{
  ...
  /* KC 8/9/14 Extension to map which types use which gravity */
  int GravityGas;          /*!< identifier of gravitational interaction to use for type 0 */ 
  int GravityHalo;         /*!< identifier of gravitational interaction to use for type 1 */ 
  int GravityDisk;         /*!< identifier of gravitational interaction to use for type 2 */ 
  int GravityBulge;        /*!< identifier of gravitational interaction to use for type 3 */ 
  int GravityStars;        /*!< identifier of gravitational interaction to use for type 4 */ 
  int GravityBndry;        /*!< identifier of gravitational interaction to use for type 5 */ 
  double SoftParam;        /*!< the scale to use in determination of the timestep for force laws not requiring artificial softening */
...
extern struct NODE
{
  ...
  /* KC 8/9/14 Note that integers are 4 bytes wide, so 8*4 = 4*4 + 4*4, and the union covers the same
     amount of memory.  If FLOAT is chosen at double precision, then there is wasted space.  
     So adding our additional summed masses and CM locations for each type should not cause any
     problems except eating more space.  The one difficulty would be if things directly index into
     byte offsets within the NODE structure, but this code would be dangerous due to the compile time
     flag that would change these offsets.  Grepping the code gives that the *PRECISION compile time flag
     is only checked during Ewald summation, so this is not (yet) a problem.
  */
  union
  {
    ...
    struct
    {
      // KC 8/10/14 Notice that we have the {x,y,z} index first, so that choosing this
      // now returns an array for the different types
      FLOAT s[3][N_GRAVS];               /*!< center of mass of node */
      FLOAT mass[N_GRAVS];               /*!< mass of node */

#ifdef NGRAVS_ACCUMULATOR
      // KC 3/11/14
      long Nparticles[N_GRAVS];       /*!< keeps track of the number of particles of each type contained within */
#endif
...
extern struct extNODE           /*!< this structure holds additional tree-node information which is not needed in the actual gravity computation */
{
  ...
  // KC 8/10/14 Uhoh. Apparently, this is needed to populate values in the tree....
  // I don't yet see why...

  FLOAT vs[3][N_GRAVS];			/*!< center-of-mass velocity */
}
...
extern struct gravdata_in
{
  union
  {
    /* KC 8/9/14 Hopefully, this is geometric center of a node and not something to do with the 
       mass center XXX */
    // KC 8/10/14 This looks like its only used for particles...
    FLOAT Pos[3];
    FLOAT Acc[3];
    FLOAT Potential;
  ...
  // KC 8/10/14 Extended to include the Mass
  // Should this be in the union or not?
  FLOAT Mass;

  // KC 12/16/14
  // Type now required regardless of whether we unequally soften or not
  int Type;
